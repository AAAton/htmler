...
// Canvas returns a Canvas. Expects width, height, ID, class
func Canvas(args ...interface{}) string {
	width := "400"
	height := "400"
	var id, class string
	if len(args) > 0 {
		width = fmt.Sprint(args[0])
	}
	if len(args) > 1 {
		height = fmt.Sprint(args[1])
	}
	if len(args) > 2 {
		id = fmt.Sprintf(`id="%s"`, args[2])
	}
	if len(args) > 3 {
		class = fmt.Sprintf(`class="%s"`, args[3])
	}
	return `<canvas ` + id + ` ` + class + ` width="` + width + `" height="` + height + `"></canvas>`
}
